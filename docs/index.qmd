...
First, however, we need to discuss the format of the data you'll be encountering. Our dataset is already quite 'tidy'. But let's apply a transformation to illustrate some of the less 'tidy' data types you'll see. We use the `pivot_wider` function to make the Region column into four separate columns, one for each region.

```{r}
subset %>% 
  pivot_wider(everything(), names_from = 'Region', values_from = 'selling_price') %>% 
  pivot_longer(c(East,Central,West,South), names_to = 'Region', values_to = 'selling_price')
```
